...
if (require.main === module) {
  ...
  */
	if (opts.derive) opts['private'] = true;
	...
	try {
		...
		console.error('sshpk-conv: error opening input file' + ': ' + e.name + ': ' + e.message);
		process.exit(1);
	}
	...
	try {
		...
		console.error('sshpk-conv: error opening output file' + ': ' + e.name + ': ' + e.message);
		process.exit(1);
	}
	...
	inFile.on('readable', function () {
		var data;
		while (data = inFile.read()) bufs.push(data);
	});
	inFile.on('end', function () {
		var buf = Buffer.concat(bufs);
		var fmt = 'auto';
		if (opts.informat) fmt = opts.informat;
		var f = sshpk.parseKey;
		if (opts['private']) f = sshpk.parsePrivateKey;
		try {
			var key = f(buf, fmt, inFileName);
		} catch (e) {
			console.error('sshpk-conv: ' + e.name + ': ' + e.message);
			process.exit(1);
		}
		...
		if (!opts.identify) {
			fmt = undefined;
			if (opts.outformat) fmt = opts.outformat;
			outFile.write(key.toBuffer(fmt));
			if (fmt === 'ssh' || !opts['private'] && fmt === undefined) outFile.write('\n');
			outFile.once('drain', function () {
				process.exit(0);
			});
		} else {
			var kind = 'public';
			if (sshpk.PrivateKey.isPrivateKey(key)) kind = 'private';
			console.log('%s: a %d bit %s %s key', inFileName, key.size, key.type.toUpperCase(), kind);
			if (key.type === 'ecdsa') console.log('ECDSA curve: %s', key.curve);
			if (key.comment) console.log('Comment: %s', key.comment);
...
//# sourceMappingURL=sshpk-conv-compiled.js.map