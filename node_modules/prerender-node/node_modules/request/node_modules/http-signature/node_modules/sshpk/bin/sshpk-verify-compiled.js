...
if (require.main === module) {
	...
	if (!opts.identity) {
		var help = parser.help({}).trimRight();
		console.error('sshpk-verify: the -i or --identity option ' + 'is required\n');
		console.error(help);
		process.exit(3);
	}
	...
	if (!opts.signature) {
		var help = parser.help({}).trimRight();
		console.error('sshpk-verify: the -s or --signature option ' + 'is required\n');
		console.error(help);
		process.exit(3);
	}
	...
	try {
		...
		console.error('sshpk-verify: error loading key "' + opts.identity + '": ' + e.name + ': ' + e.message);
		process.exit(2);
	}
	...
	try {
		...
		console.error('sshpk-verify: error parsing signature: ' + e.name + ': ' + e.message);
		process.exit(2);
	}
	...
	try {
		...
		console.error('sshpk-verify: error creating verifier: ' + e.name + ': ' + e.message);
		process.exit(2);
	}

	if (opts.verbose) {
		console.error('sshpk-verify: using %s-%s with a %d bit key', key.type, hash, key.size);
	}
	...
	try {
		...
		console.error('sshpk-verify: error opening input file' + ': ' + e.name + ': ' + e.message);
		process.exit(2);
	}
	...
	inFile.on('end', function () {
		...
		try {
			...
			console.error('sshpk-verify: error verifying data: ' + e.name + ': ' + e.message);
			process.exit(1);
		}
...
//# sourceMappingURL=sshpk-verify-compiled.js.map