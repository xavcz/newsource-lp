...
function read(buf) {
	if (typeof buf === 'string') {
		if (buf.trim().match(/^[-]+[ ]*BEGIN/)) return pem.read(buf);
		if (buf.match(/^\s*ssh-[a-z]/)) return ssh.read(buf);
		if (buf.match(/^\s*ecdsa-/)) return ssh.read(buf);
		buf = new Buffer(buf, 'binary');
	} else {
		assert.buffer(buf);
		if (findPEMHeader(buf)) return pem.read(buf);
		if (findSSHHeader(buf)) return ssh.read(buf);
	}
	if (buf.readUInt32BE(0) < buf.length) return rfc4253.read(buf);
...
//# sourceMappingURL=auto-compiled.js.map